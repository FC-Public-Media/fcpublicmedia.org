...
The wall is open. Across the top is the yellow check-in header: the code in a
black mark, *Check in*, and the header's one crooked edge at the bottom.
Below it one module fills the stage and changes by itself every 45 seconds.
...
2. **Press button "Files".**
   *You see:* Files lights, the frame is named "Files" and shows the depot,
