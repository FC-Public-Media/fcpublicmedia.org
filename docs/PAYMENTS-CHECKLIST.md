...
binding shipped as `"id": ""`, wrangler validates the whole file before running
any command, and so the placeholder locked the directory and refused
`kv namespace create` — the command that produces the id. If the namespace ever
has to be replaced, delete the block rather than blanking the id.
...
key is scoped to exactly what a stranger may cause. `site/bin/test_no_secrets.py`
...
**2.3 is why class drop-ins cannot be sold.** `site/bin/build-prices.py` refuses
...
- **Repricing existing subscribers is written**
  (`site/bin/reprice-subscriptions.py`), because Stripe pins a subscription to
