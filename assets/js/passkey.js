 ...
 * -----------------------------
 * To the person holding the device: that they hold a passkey this site issued,
 * and which site it belongs to. That is enough to show them the right form.
 *
 * To us: NOTHING. The challenge below is generated by this page, so a captured
 * assertion could be replayed and the code doing the checking is code the
 * visitor controls. This is a wayfinding step, not a security boundary, and
 * today nothing is written anywhere as a result of it.
 *
 * When the broker exists, ITS challenge replaces this one and IT verifies the
 * signature. The shape of the call stays the same; the trust does not. Do not
 * let the broker skip that because this looked like it already worked.
 *
 * Resolves to { ok: true, repo, person, credentialId } or { ok: false, reason }.
 */
export async function signIn({ rpId } = {}) {
