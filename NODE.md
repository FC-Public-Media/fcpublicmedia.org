...
tests/               browser tests.
*.md, advocate.yml   the documents, and the seat declarations.
```
...
| what | from | into |
|---|---|---|
| `script/sync-cablecast.py` | the Cablecast catalog | `site/_data/cablecast.json` |
| `script/sync-schedule.py` | the broadcast schedule | `site/_data/airings.json` |
| `script/sync-calendar.py` | the class calendar | `site/_data/calendar.json` |
| `script/sync-feeds.py` | member program feeds | `site/_data/member_programs.json` |
| `script/sync-nonprofits.py` | the nonprofit registry | `site/assets/nonprofits.json` |
...
of its own, and already checked on every push by `script/check-template.py` —
...
theme gems on purpose; a staging step in `script/` is the option that matches
